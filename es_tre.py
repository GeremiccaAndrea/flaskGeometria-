...
@app.route('/')
def es_tre():
    return render_template('es_tre.html')


@app.route('/data', methods = ['GET'])
def geo():
    geometria = float(request.args.get("geometria"))
    base = float(request.args.get('base'))
    altezza = float(request.args.get('altezza'))
    if calcolo == "Area":
        area = base * altezza
        return render_template('es3area.html',Base = base,Altezza=altezza, Area = area)
    else:
        diagonale = math.sqrt(base ** 2 + altezza ** 2)
        return render_template('es3diagonale.html',Base = base,Altezza=altezza, Diagonale = diagonale)
